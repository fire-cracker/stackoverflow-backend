import {
  ...
  fetchVote, upVote, votesCount, downVote, fetchSubscription, subscribe
...
* @returns {Object} JSON object (JSend format)
*/
export const subscribeQuestion = async (req, res) => {
  try {
    const {
      params: {
        questionId
    ...
    if (String(question.userId) === String(userId)) {
      return res.status(401).send({
        status: 'fail',
        data: {
          message: 'You cannot subscribe to your question',
        }
      });
    }

    const subcription = await fetchSubscription(questionId, userId)

    if (subcription) {
      return res.status(400).send({
        status: 'fail',
        data: {
          message: 'You already subscribed to this question',
        }
      });
    }

    await subscribe(questionId, userId);

    return res.status(200).send({
      status: 'success',
      data: {
        message: 'You have subscribed to this question'
      }
    });
  } catch (error) {
    return res.status(502).send({
